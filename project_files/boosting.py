...
# rectanglefilter2(h,w)
#      parameters:
#         - make make left and right margin of filter of size w
#         - make filter height of h
#     returns filter of size
#         (h,3*w)
#             -
...
# same thing as rectangle filter 3 , but vertically
...
def integral_image(image):
    ...
    # print("",np.cumsum(image, axis=0))
    # sum all values along vertical first
    # then sum all values across the horizontal
...
def weighted_error(responses, labels, weights, classifier):
    ...
    best_threshold = None
    best_direction = None
    arrArrange =  np.arange(minimum, maximum + step, step)
    
    # print( f'arrArrange: {arrArrange}')
    
    # print(f'inside weighted errro')
    # print(responses.shape, labels.shape, weights.shape, classifier,"\n")
    # print(f'arrArrange.shape: {arrArrange.shape}')

    for threshold in arrArrange:
        
        thresholded = (classifier_responses > threshold).astype(np.float32)
        thresholded[thresholded == 0] = -1
        # print(labels.shape,threshold.shape)
...
def adaboost(responses, labels, rounds):
    ...
    Args:
        responses(classifiers) (numpy.ndarray): The input data matrix of shape (n_samples, n_features).
    ...
    Returns:
        ...
        split the data.
    """
    lengthResponses = responses.shape[0]
    # vector of weights that corrrespond to each classfier in responses
    weights = np.ones(lengthResponses) / lengthResponses
    boosted_responses = np.zeros(lengthResponses)
    ...
    for round in range(rounds):
        ...
        print(
            f"Round: {round + 1}, Error: {error}, Best Error: {best_error}, "
